...
class IpaSpider(CrawlSpider):
    ...
    rules = (
        #Rule(LinkExtractor(allow=r'/security/announce/announce.html')),
        #Rule(LinkExtractor(allow=r'/security/announce/alert.html')),
...
#        item['text'] = response.css('.unit').xpath('string()').get().replace('　',' ').strip()
